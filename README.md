...
目前開發來源正在準備 `0.2.0` 正式版；公開下載仍是已完成獨立驗證的 `0.2.0-rc.3`，直到正式版
以獨立 commit、digest、主機與 Windows Sandbox 證據完成並另行核准發布。
...
Windows Sandbox gate。`0.2.0` 必須取得自己的證據，不能沿用 RC3。完整驗證、證據與回復流程見
